                            ...
                            <select name="hora" id="hora"
                                ...
                                @for($h = 8; $h <= 18; $h++)
                                    @foreach(['00', '30'] as $m)
                                        ...
                                        <option value="{{ $time }}" {{ old('hora', substr($cita->hora, 0, 5)) == $time ? 'selected' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                @foreach($servicios as $servicio)
                                    @php $seleccionado = in_array($servicio->id, old('servicios', $serviciosSeleccionados)); @endphp
                                    <label class="flex items-center p-3 border rounded-lg hover:bg-blue-50 transition cursor-pointer {{ $seleccionado ? 'bg-blue-50 border-blue-400' : 'border-gray-200' }}">
